...
function ProductTable() {
  ...
  const fetchData = async () => {
    try {
      const fetchProducts = await requestData(
  ...
  const handleInputChangeEditingProduct = (
  ...
  const handleAddProduct = async (productToAdd: Product) => {
    ...
    try {
      const addedProduct = await requestNewProduct(
        `${import.meta.env.VITE_API_URL}/products`,
        productToAdd,
      )
  ...
  const handleAddProducts = async (productsToAdd: Product[] | Product) => {
    try {
      ...
      const addedProducts = await requestNewProduct(
        `${import.meta.env.VITE_API_URL}/products`,
        requestBody,
      )
  ...
  const handleSaveEdit = async () => {
    try {
      await requestProductUpdate(
        `${import.meta.env.VITE_API_URL}/products/${editingProductId}`,
        editingProduct,
      )
      ...
      <div className="relative overflow-x-auto w-8/12">
        <div className="mt-4 flex">
          <input
            ...
            className="mr-2 px-2 py-1 rounded border-gray-300 focus:outline-none focus:border-blue-500"
                ...
                <td className="px-6 py-4">
                  {editingProductId === product.id ? (
                    <input
                      ...
                      onChange={handleInputChangeEditingProduct}
                    />
                  ) : (
                    product.name
                  )}
